...
export const ProductProvider = ({children}) =>{
    ...
    const updateBag = (item) =>{
        ...
        setBag(prev =>{
            ...
            if(!current.cart) return [...uniqueEntries, {name: duplicates[0].name, quantity: duplicates[0].quantity + current.quantity, price: current.price, imageUrl: duplicates[0].imageUrl}]
        }) 
    }
    ...
    return <ProductContext.Provider value={{products, bag, updateBag, deleteBagItem, quantityBagUpdate, bagTotal, setBagTotal, loading}}>
        {children}
