...
const SideBarItem = (props) => {
  ...
  useEffect(() => {
    ...
    props.setBagItems((prev) => {
      ...
      return [
        ...uniqueEntries,
        { name: props.name, total: quantity * props.price },
      ];
    });
  }, [quantity]);
  ...
  return (
    <div
      className="grid grid-cols-2 gap-2 px-2 border-b-2 border-slate-300 py-4"
      key={props.name}
    >
      ...
      <div className="flex-col">
        <div className="flex justify-between">
          ...
          <button onClick={deleteItemHandle}>
            <Trash />
          </button>
        </div>
        {props.products.length > 0 ? (
          <div>
            <h3 className="text-md font-semibold dom_prize">
              R {separateNumber(props.price * quantity)}
            </h3>
            <h3 className="text-sm py-2 italic">
              <span>R </span>
              {separateNumber(props.price)} X {quantity}
            </h3>
          </div>
        ) : (
          <p>Loading...</p>
