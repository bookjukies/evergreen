...
const Shipping = () => {
  ...
  const move = () => {
    navigate("/checkout/payment", {
      state: { details: location.state, bag: bag, cartTotal: cartTotal },
    });
  };
  if (bagTotal === 0) {
    navigate("/");
    ...
    <div className="w-screen px-4 grid grid-cols-1 place-content-center md:w-full">
      <div className="py-4 border-2 border-gray-400 px-2 rounded-md mb-8">
        ...
        <div className="mt-4 ">
          ...
          <p>
            {location.state.data.city} {location.state.data.adress_line_1}{" "}
            {location.state.data.adress_line_2}
          </p>
        </div>
      </div>
      ...
      <form action="">
        <div className="border flex px-2 py-3 rounded border-gray-400 w-full">
          ...
          />
           {" "}
          <label for="delivery" className="w-full flex px-2">
            <span className="grow">Delivery</span>
            <span className="text-right">R 100.00</span>
          </label>
        </div>
      </form>
      <button
        className="bg-black text-white w-full place-self-center p-2 rounded my-8 "
        onClick={move}
      >
        Continue to Payment
      </button>
