...
const Payment = () => {
  ...
  const test = async () => {
    ...
    const res = await axios.post("https://mimi-client.onrender.com/pay", {
      id,
      cartTotal,
      bag: data[0].bag,
      details: data[0].details.data,
    });
    setForm(res.data);
  };
    ...
    <div className="h-cover w-screen px-6 md:w-full">
      <div className="py-4 border-2 px-2 rounded-md mb-8 border-gray-400">
        ...
        <div className="mt-4 border-b-2 border-b-gray-400">
          ...
          <p>
            {location.state.details.data.city}{" "}
            {location.state.details.data.adress_line_1}{" "}
            {location.state.details.data.adress_line_2}
          </p>
        </div>
        <div className="mt-4">
      ...
      <div className="py-4 ">
        ...
        <div className="flex border rounded align-middle border-gray-400">
          <img
            className="w-12"
            src="/images/visa.png"
            alt="payment procesor visa"
          />
          <img
            className="w-12"
            src="/images/master.png"
            alt="payment procesor mastercard"
          />
        </div>
      </div>
        ...
        <form action="">
          <div className="border flex px-2 py-4 rounded border-gray-400">
            <input
              ...
              id="different_adress"
              name="billing_adress"
              value="different_adress"
            />
             {" "}
            <label for="different_adress" className="flex ">
              Use Different Adress
            </label>
          </div>
        </form>
      </div>
      ...
      <div
        className="bg-black text-white w-full capitalize py-2 my-8 md:w-full md:px-32 md:mt-4 text-center"
